                   ...
                   int yo, int np)
{
  ...
  Complejo z0;
  double ix, iy;
  int siguiente_fila, num_fila, filas_hechas, proc, acabat;
  ...
  if(yo == 0) {
    ...
    for ( filas_hechas = 0 ; filas_hechas < h ; filas_hechas++ ) {
      ...
      MPI_Test(&req, &acabat, &status);

      while ( !acabat && filas_hechas < h ) {
        printf("Entra, %d  %d\n", acabat, status);
	      /* Calcula esa fila */
