...
int nLU(double **ppdMat, int nN, int nId, int nP, int nBloque)
{
  ...
  for (k = 0; k < nN - 1; k++) {
    ...
    if (k % nP == nId /* k / nBloque*/) {
      if (fabs(ppdMat[k / nP /* k % nBloque*/][k]) < EPSILON) return -1;
      memcpy(pdPivote, ppdMat[k / nP /* k % nBloque */], nN * sizeof(double));
    }

    MPI_Bcast(pdPivote, nN, MPI_DOUBLE, k % nP/*k / nBloque*/, MPI_COMM_WORLD);

    for (i = k + 1; i < nN; i++) {

      if (i % nP == nId /*i / nBloque*/) {// comproba si la fila es meua
        //modificar
        ppdMat[i / nP /*i % nBloque*/][k] = ppdMat[i / nP /*i % nBloque*/][k] / pdPivote[k];
        for (j = k + 1; j < nN; j++)
          ppdMat[i / nP /*i % nBloque*/][j] -= ppdMat[i / nP /*i % nBloque*/][k] * pdPivote[j];
      }
    }
  }
  free(pdPivote);
...
int nElimProg(double **ppdL, double *pdB, int nN, int nId, int nP, int nBloque)
{
  ...
  for (i = 0; i < nN; i++) {
    MPI_Bcast(&pdB[i], 1, MPI_DOUBLE, i % nP /*i / nBloque*/, MPI_COMM_WORLD);
    for (j = i + 1; j < nN; j++) {
      if (j % nP/*j / nBloque*/ == nId)
        pdB[j] -= ppdL[j / nP/*j % nBloque*/][i] * pdB[i];
    }
  }
  return 0;
}
...
int nSustReg(double **ppdU, double *pdB, int nN, int nId, int nP, int nBloque)
{
  ...
  for (i = nN - 1; i >= 0; i--) {
    if (i % nP/*i / nBloque*/ == nId) {
      if (fabs(ppdU[i / nP/*i % nBloque*/][i]) < EPSILON) return -1;
      pdB[i] = pdB[i] / ppdU[i / nP/*i % nBloque*/][i];
    }
    MPI_Bcast(&pdB[i], 1, MPI_DOUBLE, i % nP/*i / nBloque*/, MPI_COMM_WORLD);
    for (j = i - 1; j >= 0; j--) {
      if (j % nP/*j / nBloque*/ == nId)
        pdB[j] -= ppdU[j / nP/*j % nBloque*/][i] * pdB[i];
    }
  }
  return 0;
}
...
int main(int argc, char *argv[])
{
  ...
  MPI_Status status;
  MPI_Datatype files;
  MPI_Type_vector(nBloque, nN, nN * nP, MPI_DOUBLE, &files);
  MPI_Type_commit(&files);


  /*for(i = 0; i < nN && nId == 0; i+=nP){
    // Copy row of p0
    memcpy(ppdMatLcl[i/nP], ppdMat[i], nN * sizeof(double) );
  }*/

  if ( nId == 0)
      MPI_Sendrecv(ppdMat[0], 1, files, 0, 300, ppdMatLcl[0], nBloque * nN, MPI_DOUBLE, 0, 300, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

  /* Communicate rows */
  for(i = 1; i < nP; i++){

    if ( nId == 0){
      MPI_Send(ppdMat[i], 1, files, i, 0, MPI_COMM_WORLD);
    }else if ( nId == i ) {
      MPI_Recv(ppdMatLcl[0], nBloque * nN, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
  }
